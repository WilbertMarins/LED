...
%token NUMBER
%token PLUS MINUS TIMES DIVIDE POWER TRIANGLE CIRCLE
%token LEFT RIGHT
%token END
...
Expression: NUMBER { $$=$1; };
Expression: Expression PLUS Expression { $$ =$1 + $3; printf("%.2f + %.2f\n", $1, $3); };
Expression: Expression MINUS Expression { $$ = $1 - $3; printf("%.2f - %.2f\n", $1, $3); };
Expression: Expression TIMES Expression { $$ = $1 * $3; printf("%.2f * %.2f\n", $1, $3); };
Expression: Expression DIVIDE Expression { $$ = $1 / $3; printf("%.2f / %.2f\n", $1, $3); };
Expression: MINUS Expression %prec NEG { $$ = -$2; printf("- %.2f\n", $2); };
Expression: Expression POWER Expression { $$ = pow($1, $3); printf("%.2f ^ %.2f\n", $1, $3); };
Expression: LEFT Expression RIGHT { $$ = $2; };
Expression: Expression TRIANGLE Expression { $$ = sqrt(pow($1, 2) + pow($3, 2)); printf("Triangulo retangulo:\nADJACENTE:%.2f\nOPOSTO:%.2f\nHIPOTENUSA:%.2f\n", $1, $3, $$); };
